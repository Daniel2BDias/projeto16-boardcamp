...
export const registerNewClient = async (req, res) => {
  ...
  try {
    const alreadyRegistered = await db.query(`SELECT * FROM customers WHERE cpf=$1;`, [cpf]);
...
export const editClientRegistry = async (req, res) => {
  ...
  try {
    const cpfConflict = await db.query(`SELECT * FROM customers WHERE cpf=$1;`, [cpf]);
    if(cpfConflict?.rows?.cpf) return res.sendStatus(409);

    await db.query(`UPDATE customers SET name=$2, cpf=$3, birthday=$4, phone=$5 WHERE id=$1;`, [id, name, cpf, birthday, phone]);
    res.sendStatus(200);
  } catch (error) {
    res.status(500).send(error.message);
  }
